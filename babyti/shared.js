...
const PERSONALITIES = {
  LL: { // Low Surgency · Low Sensitivity
    ...
    desc:'자극에 쉽게 흔들리지 않고 자기만의 페이스로 세상을 살아가는 아기예요. 시끄러운 환경에서도 평온함을 유지하고 변화 앞에서도 의연하게 대처해요. 환경 민감성 연구(Pluess 2015)에서 \'민들레(Dandelion)\' 타입으로 분류돼요. 척박한 곳에서도 잘 자라는 민들레처럼, 어떤 환경에서도 안정적으로 잘 자라는 단단한 기질이에요.',
  ...
  LM: { // Low Surgency · Mid Sensitivity
    ...
    desc:'행동보다 관찰을 먼저 하는, 머릿속에서 많은 일이 일어나는 아기예요. 익숙해질 때까지 충분히 살피고 한 가지에 깊이 빠져드는 경향이 있어요. 영아 기질 연구(Rothbart IBQ-R)에서 \'외향성은 낮지만 주의 집중력은 중간 이상\'인 \'조용한 관찰자\' 패턴으로 분류돼요. 또래보다 사고력과 언어 능력이 일찍 꽃피는 타입이에요.',
  ...
  LH: { // Low Surgency · High Sensitivity
    ...
    desc:'작은 소리, 미세한 표정 변화까지 모두 느끼는 풍부한 감수성의 소유자예요. 자극에 예민한 건 \'약해서\'가 아니라 \'세상을 더 깊이 느끼기 때문\'이에요. 감각 처리 민감성 연구(Aron 2012)에서는 \'난초(Orchid)\' 타입으로 분류돼요. 난초가 좋은 환경에서만 화려하게 피어나듯, 따뜻하고 안정된 환경에서 누구보다 풍부한 정서와 창의력을 꽃피우는 기질입니다.',
  ...
  ML: { // Mid Surgency · Low Sensitivity
    ...
    desc:'새로운 상황에도 금세 적응하고 변화를 즐기는 아기예요. 잘 울지 않고 어디서나 즐거움을 찾아요. 1970년대 토머스·체스 연구에서 \'순한 아기(Easy baby)\'로 불리던 유형 — 잘 먹고 잘 자고 잘 적응하는 타입에 적당한 활동성과 모험심이 더해진 균형형이에요. 양육자에게 가장 큰 안정감을 주는 기질로 알려져 있습니다.',
  ...
  MM: { // Mid · Mid
    ...
    desc:'외향성과 민감성이 모두 적당한, 어떤 환경에서도 무난히 잘 어울리는 만능형 아기예요. 환경 민감성 후속 연구(Lionetti 2018)의 \'튤립(Tulip)\' 타입에 해당해요. 강인한 민들레와 섬세한 난초 사이에서 가장 유연하게 적응하는 균형형 기질로, 전체 아기의 약 40%가 여기 속할 정도로 가장 흔한 타입이에요.',
  ...
  MH: { // Mid Surgency · High Sensitivity
    ...
    desc:'다른 사람의 표정과 감정을 빠르게 감지하고 반응하는 정서 지능이 높은 아기예요. 환경 민감성 연구(Pluess 2015)에서 \'사회·정서적 단서에 특히 민감한\' 그룹으로 분류돼요. 부모의 작은 표정 변화까지 흡수하기 때문에 따뜻하고 안정된 환경에서 자라면 뛰어난 사회성과 공감 능력을 가진 \'타고난 리더\'로 성장합니다.',
  ...
  HL: { // High Surgency · Low Sensitivity
    ...
    tags:['#추진력','#용기','#독립심','#리더십'],
    desc:'두려움이 적고 호기심은 강한, 어디든 먼저 뛰어드는 행동파 아기예요. 자극에 둔감해 좀처럼 위축되지 않아요. 영아 기질 연구(Rothbart)에서 \'외향성은 높고 두려움·불안은 낮은\' 드문 조합 — 새로운 일에 망설임 없이 뛰어들면서도 좌절을 빠르게 털어내는, 미래의 개척자 자질을 가진 기질이에요.',
  ...
  HM: { // High Surgency · Mid Sensitivity
    ...
    desc:'활발하면서도 상황을 잘 파악하는, 타고난 리더 기질의 아기예요. 다른 사람의 관심을 받는 걸 즐기고 분위기를 잘 이끌어요. \'높은 외향성 + 적당한 민감성\'의 균형 — 자기 표현은 풍부하면서도 상대의 반응을 잘 읽는 이 조합은, 영아 기질 연구에서 사회적 성공과 가장 강한 상관을 보이는 패턴으로 알려져 있어요.',
  ...
  HH: { // High Surgency · High Sensitivity
    ...
    desc:'외향성과 민감성이 모두 높은 드문 조합! 강렬한 감정과 풍부한 표현력, 폭발적인 호기심을 가진 아기예요. 벨스키·플뤼스(2009)의 \'차별적 민감성(Differential Susceptibility)\' 이론에 따르면, 이 타입은 환경의 영향을 가장 크게 받는 \'고반응형\' 기질이에요. 좋은 환경에서는 가장 뛰어난 성취를 보이는 동시에, 환경이 거칠면 가장 큰 어려움을 겪을 수 있어 양육 환경이 결정적입니다.',
...
function showPage(id) {
  ...
  // Back button lives outside .page (below Kakao AdFit) — visible only on quiz.
